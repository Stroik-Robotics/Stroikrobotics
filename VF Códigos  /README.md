...
# =============================================================================
# CÓDIGO UNIFICADO: ABIERTA Y CERRADA
# =============================================================================

# -----------------------------------------------------------------------------
# 1. PANEL DE CONTROL SÚPER OPTIMIZADO (ABIERTA)
# -----------------------------------------------------------------------------
VELOCIDAD_BASE_ABIERTA = -1000  # Fluidez total
...
SIGNO_DIRECCION_ABIERTA = 1
TOTAL_GIROS             = 12

RELACION_ENGRANAJE_ABIERTA = 5
ANGULO_MAX_RUEDAS_ABIERTA  = 18
ANGULO_MOTOR_GIRO_ABIERTA  = ANGULO_MAX_RUEDAS_ABIERTA * RELACION_ENGRANAJE_ABIERTA


# -----------------------------------------------------------------------------
# 2. CONFIGURACIÓN Y HARDWARE (CERRADA)
# -----------------------------------------------------------------------------
ev3             = EV3Brick()
...
# =============================================================================
# 3. FUNCIONES GENERALES Y DE LA MODO CERRADA
# =============================================================================
...
# =============================================================================
# 4. FUNCIONES DE EJECUCIÓN (MODO ABIERTA)
# =============================================================================

def ejecutar_giro_abierta(sentido, giro_en_vuelta):
    ev3.light.on(Color.RED)
    ev3.speaker.beep(frequency=800, duration=60)

    angulo_volante = ANGULO_MOTOR_GIRO_ABIERTA * sentido * SIGNO_DIRECCION_ABIERTA
...
# =============================================================================
# 5. MENÚ DE SELECCIÓN DE PISTA (ARRANQUE INTERactivo)
# =============================================================================
ev3.light.on(Color.ORANGE)
ev3.speaker.beep(frequency=600, duration=150)
print("SELECCIONA MODO DE PISTA:")
print("ARRIBA/ABAJO para cambiar, CENTRO para confirmar.")
print("Verde = Abierta | Rojo = Cerrada")
...
while True:
    ...
    presionado = ev3.buttons.pressed()
    if len(presionado) > 0:
        if Color.YELLOW in presionado or 'Up' in str(presionado) or 'Down' in str(presionado):
            modo_abierta = not modo_abierta
            ev3.speaker.beep(frequency=1000, duration=500)
            wait(300)
        elif Color.BLACK in presionado or 'Center' in str(presionado):
...
# =============================================================================
# 6. EJECUCIÓN SEGÚN MODO SELECCIONADO
# =============================================================================
...
if modo_abierta:
    ...
    motor_traccion.run(VELOCIDAD_BASE_ABIERTA)
    ...
    while giros_completados < TOTAL_GIROS:
        ...
        if suma_distancias >= UMBRAL_PASILLO_MM:
            ...
            ejecutar_giro_abierta(sentido_pista, giro_en_vuelta)
        ...
        correccion       = (KP_US_ABIERTA * error_pos) + (KI_US_ABIERTA * integral_us) + (KD_US_ABIERTA * derivada_us)
        correccion_motor = correccion * SIGNO_DIRECCION_ABIERTA
        limite_motor     = ANGULO_MAX_RUEDAS_ABIERTA * RELACION_ENGRANAJE_ABIERTA
    ...
    print("CARRERA ABIERTA COMPLETADA -", giros_completados, "giros ejecutados.")
    ...
    grados_bloqueo_giro = (DISTANCIA_BLOQUEO_POST_GIRO_CM * 10 / (3.14159 * DIAMETRO_RUEDA_MM)) * 360
    ...
    while True:
        ...
        if pasillo_confirmado and abierto and post_giro_libre:
            ...
            if lecturas_esquina >= 1:
                ...
                angulo_bloqueo_av = gyro.angle()
                avanzar_recto_pid(angulo_bloqueo_av, dist_av, vel_pos)

                inv        = 1 if es_normal else -1
                timon_giro = (ANGULO_TIMON_MAX_GIRO * RELACION_ENGRANAJE * SIGNO_DIRECCION * dir_esquina * inv)
                ...
                while t < TIMEOUT_GIRO_PRINCIPAL_MS:
                    ...
                    vel_g = (VELOCIDAD_CRUCERO if dif > 25 else max(int(VELOCIDAD_CRUCERO * 0.3), int(VELOCIDAD_CRUCERO * dif / 25)))
            ...
            if firma in (1, 2):
                ...
                if carril_actual == carril_obj:
                    ...
                    while t < TIMEOUT_ESQUIVE_CARRIL_MS:
                        ...
                        lect     = (sensor_izq.distance() + OFFSET_IZQ_MM if carril_obj == CARRIL_IZQ else sensor_der.distance() + OFFSET_DER_MM)
                        if lect <= lim_desc:
